...
def parse_directions(direc):
    grid = []
    curr = [0, 0]
    dst = 0
    for i in direc:
        if i[0] == 'R':
            for j in range(curr[0] + 1, curr[0] + int(i[1:]) + 1):
                dst += 1
                grid.append(([j, curr[1]], dst))
            curr[0] = j
        elif i[0] == 'L':
            for j in range(curr[0] - int(i[1:]), curr[0]):
                dst += 1
                grid.append(([j, curr[1]], dst))
            curr[0] = curr[0] - int(i[1:])
        elif i[0] == 'U':
            for j in range(curr[1] + 1, curr[1] + int(i[1:]) + 1):
                dst += 1
                grid.append(([curr[0], j], dst))
            curr[1] = j
        else:
            for j in range(curr[1] - int(i[1:]), curr[1]):
                dst += 1
                grid.append(([curr[0], j], dst))
            curr[1] = curr[1] - int(i[1:])
    return grid
